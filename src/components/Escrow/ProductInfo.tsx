...
export default function ProductInfo({ data }: { data: any }) {
  ...
  const modal = useModal('family-confirm-modal');

  const { confirmSent } = useMarketplace(data?.collection);
    ...
    <>
      <div className="flex flex-col h-full gap-[50px]">
        ...
        <div className="flex flex-col gap-2">
          ...
          {data?.seller === account && !data?.trackingId && (
            <Button
          ...
          {data?.buyer === account && data?.trackingId && (
            <Button
              // style={{ width: "286px", height: "40px" }}
              variant="dark"
              onClick={() =>
                modal.show({
                  tokenId: TokenId.parseTokenId(data?.tokenId),
                  data,
                })
              }
            >
